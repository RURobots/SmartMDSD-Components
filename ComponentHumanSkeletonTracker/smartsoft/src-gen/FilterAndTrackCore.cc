...
FilterAndTrackCore::FilterAndTrackCore(Smart::IComponent *comp, const bool &useDefaultState) 
...
FilterAndTrackCore::~FilterAndTrackCore()
{
}


void FilterAndTrackCore::notify_all_interaction_observers() {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	// try dynamically down-casting this class to the derived class 
	// (we can do it safely here as we exactly know the derived class)
	if(const FilterAndTrack* filterAndTrack = dynamic_cast<const FilterAndTrack*>(this)) {
		for(auto it=interaction_observers.begin(); it!=interaction_observers.end(); it++) {
			(*it)->on_update_from(filterAndTrack);
		}
	}
}

void FilterAndTrackCore::attach_interaction_observer(FilterAndTrackObserverInterface *observer) {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	interaction_observers.push_back(observer);
}

void FilterAndTrackCore::detach_interaction_observer(FilterAndTrackObserverInterface *observer) {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	interaction_observers.remove(observer);
}

int FilterAndTrackCore::execute_protected_region()
{
...
void FilterAndTrackCore::updateAllCommObjects()
{
...
Smart::StatusCode FilterAndTrackCore::humanSkeletonsPushServiceOutPut(DomainHumanTracking::CommHumanPositionsAndVelocities &humanSkeletonsPushServiceOutDataObject)
{
	Smart::StatusCode result = COMP->humanSkeletonsPushServiceOut->put(humanSkeletonsPushServiceOutDataObject);
	if(useLogging == true) {
		//FIXME: use logging
...
void FilterAndTrackCore::triggerLogEntry(const int& idOffset)
{
	if(useLogging == true) {
...
int FilterAndTrackCore::getPreviousCommObjId()
{
